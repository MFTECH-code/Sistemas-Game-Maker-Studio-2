poder = global.item_index[# arma, item_stat.dano];
...
switch(estado)
{
	...
	case "ataque":
		...
		if (atacando == false)
		{	
			// Definindo o dano com base no item equipado
			...
			// Vamos definir a posição onde a hitbox, será criada utilizando lengthdir
			var xx = (x - sprite_xoffset) + sprite_width / 2; // Centro da sprite
			var yy = (y - sprite_yoffset) + sprite_height / 2; // Centro da sprite
			
			xx += lengthdir_x(sprite_get_width(spr_player) / 2, face * 90); 
			yy += lengthdir_y(sprite_get_height(spr_player) / 2, face * 90);
...
x += velh;
y += velv;
// Garantindo que a hurtbox fique no player
hurtbox.x = x;
hurtbox.y = y;