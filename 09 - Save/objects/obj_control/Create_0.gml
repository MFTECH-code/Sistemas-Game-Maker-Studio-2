...
ini_open("save.ini");
var ds_inv = ini_read_string("itens", "inv", "vazio");
if (instance_exists(obj_player))
{
	var player_max_vida = ini_read_real("player", "max_vida", 0);
	var player_vida = ini_read_real("player", "vida", 0);
	var player_arma = ini_read_real("player", "arma", 0);
	var player_pos_x = ini_read_real("player", "pos_x", room_width / 2);
	var player_pos_y = ini_read_real("player", "pos_y", room_height / 2);
}
else
{
...
// Colocando os valores salvos em seus lugares
ds_grid_read(global.inv, ds_inv);
if (instance_exists(obj_player))
{
	// Checando se ja havia salvo algum valor antes
	if (player_max_vida > 0)
	{
		obj_player.max_vida = player_max_vida;
		obj_player.vida = player_vida;
		obj_player.arma = player_arma;
		obj_player.x = player_pos_x;
		obj_player.y = player_pos_y;
	}
}
